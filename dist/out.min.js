var COIN_SYMBOL=null;var DRAW_SCORE=true;var CENTER_SCORE=false;var ALLOW_JUMPING=true;var FLATTEN_AT_ZERO=false;var ZERO=999;var FIX_VIEWPORT_WIDTH=0;var FIX_VIEWPORT_HEIGHT=0;var FIX_COIN_SCALE=0;var DEADLY_SPIKES=true;var FLATTEN_SPIKES=true;var DEBUG_COLLISIONS=false;var SLIGHTLY_SAFER_COLLISIONS=false;var SLIGHTLY_SAFER_DEATHS=false;var GROUND_COLOR="#fff";var COIN_COLOR="#ff9";var BACKGROUND_COLOR="#033";var MAX_MILLISECONDS_PER_FRAME=66;var GOOD_TOUCH=true;var OK_TOUCH=false;
var RESTITUTION=.3;var DISABLE_GRADIENT_EQUALITY_CHECK=true;var LEVEL_GENERATION_SCALE_FACTOR=2;if(FIX_VIEWPORT_HEIGHT&&FIX_VIEWPORT_WIDTH){a.width=FIX_VIEWPORT_WIDTH;a.height=FIX_VIEWPORT_HEIGHT}else if(FIX_VIEWPORT_WIDTH){a.height=FIX_VIEWPORT_WIDTH*a.height/a.width;a.width=FIX_VIEWPORT_WIDTH}x=a.width;y=0;v=0;w=0;t=0;T=0;L=0;g=0;if(FIX_COIN_SCALE)r=FIX_COIN_SCALE;else r=a.width/99;
if(ALLOW_JUMPING)if(GOOD_TOUCH)onmousedown=a.ontouchstart=function(){T=t};else if(OK_TOUCH)onmousedown=a.onclick=function(){T=t};else a.onclick=function(){T=t};s=[{x:0,y:0,u:0,v:0,a:0}];
...
Math.cos(l.a))*RESTITUTION;else n=0;v=m*Math.cos(l.a)-n*Math.sin(l.a);w=m*Math.sin(l.a)+n*Math.cos(l.a);o=x+D-q;p=y+U-k;m=o*Math.cos(l.a)+p*Math.sin(l.a);if(SLIGHTLY_SAFER_COLLISIONS)n=-Math.sqrt(Math.abs(m))-.1;else n=-1;x=q;y=k;D=m*Math.cos(l.a)-n*Math.sin(l.a);U=m*Math.sin(l.a)+n*Math.cos(l.a);L=now;if(DEADLY_SPIKES&&l.d){x=a.width;y=0;if(SLIGHTLY_SAFER_DEATHS)break}}}}}}c.stroke();x+=D;y+=U;if(ALLOW_JUMPING&&T&&Math.abs(T-L)<299){if(FIX_COIN_SCALE)w=-FIX_COIN_SCALE/99;else w=-r/33;T=0;L=0}if(DEBUG_COLLISIONS){c.strokeStyle=
"#F00";c.beginPath();j=0;do{l=s[j];j++;c.lineTo(l.u-X,l.v-Y);nextPoint=l}while(nextPoint.x>X);c.stroke()}c.beginPath();c.arc(x-X,y-Y,r,0,7);if(COIN_COLOR){c.fillStyle=COIN_COLOR;c.fill();if(DRAW_SCORE)if(CENTER_SCORE){c.textAlign="center";c.fillText("$"+(ZERO-(y/r|0)),x-X,y-Y-r*2)}else c.fillText("$"+(ZERO-(y/r|0)),x-X-r,y-Y-r*2);c.fillStyle=BACKGROUND_COLOR;if(COIN_SYMBOL)c.fillText(COIN_SYMBOL,x-X-r/2,y-Y+r/2)}else{if(BACKGROUND_COLOR)c.fillStyle=BACKGROUND_COLOR;c.stroke()}})(0);
