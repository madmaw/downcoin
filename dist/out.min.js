...
x>a.width?v*r/9+(l.v-y)*4/a.width-r/50:0;else j=x>a.width?v+(l.v-y-20)/99:0;if(DEADLY_SPIKES)m=0;if(DEADLY_SPIKES&&l.d>1){o=Math.PI/3;m=1}else if(DEADLY_SPIKES&&Math.random()<(.3-Math.abs(l.a))*Math.sqrt(x)/999&&!l.d){o=-Math.PI/3;m=2}else if(FLATTEN_AT_ZERO&&k>ZERO/r)o=0;else o=Math.min(Math.max(l.d?0:l.a+(Math.random()-.5)*Math.PI/9,-Math.PI/9*j),Math.PI*(x+9999)/99999*(1-j));p=(l.a+o)/2-Math.PI/2;if(DEADLY_SPIKES)l={x:q,y:k,u:q+Math.cos(p)*r,v:k+(m>0?0:Math.sin(p)*r),a:o,d:m};else l={x:q,y:k,u:q+
Math.cos(p)*r,v:k+Math.sin(p)*r,a:o}}c.strokeStyle=GROUND_COLOR;c.beginPath();for(j=0;!nextPoint||nextPoint.x>X;nextPoint=l){l=s[j++];c.lineTo(l.x-X,l.y-Y);O=l.a-Math.PI/2;Q=Math.abs(O-Math.atan2(U,D));if(nextPoint&&Q>Math.PI/2&&Q<Math.PI*3/2){K=Math.tan(l.a);G=U/D;if(DISABLE_GRADIENT_EQUALITY_CHECK||K!=G){Z=l.v-K*l.u;if(v)q=(y-G*x-Z)/(K-G);else q=x;if(q<nextPoint.u&&q>=l.u){k=K*q+Z;e=x-q;z=y-k;if(e*e+z*z<D*D+U*U){m=v*Math.cos(l.a)+w*Math.sin(l.a);if(RESTITUTION)n=(v*Math.sin(l.a)-w*Math.cos(l.a))*
RESTITUTION;else n=0;v=m*Math.cos(l.a)-n*Math.sin(l.a);w=m*Math.sin(l.a)+n*Math.cos(l.a);o=x+D-q;p=y+U-k;m=o*Math.cos(l.a)+p*Math.sin(l.a);if(SLIGHTLY_SAFER_COLLISIONS)n=-Math.sqrt(Math.abs(m))-.1;else n=-1;x=q;y=k;D=m*Math.cos(l.a)-n*Math.sin(l.a);U=m*Math.sin(l.a)+n*Math.cos(l.a);L=now;if(DEADLY_SPIKES&&l.d){x=a.width;y=0}}}}}}c.stroke();x+=D;y+=U;if(ALLOW_JUMPING&&T&&Math.abs(T-L)<299){if(FIX_COIN_SCALE)w=-FIX_COIN_SCALE/30;else w=-r/30;T=0;L=0}if(DEBUG_COLLISIONS){c.strokeStyle="#F00";c.beginPath();
j=0;do{l=s[j];j++;c.lineTo(l.u-X,l.v-Y);nextPoint=l}while(nextPoint.x>X);c.stroke()}c.beginPath();c.arc(x-X,y-Y,r,0,Math.PI*2);if(COIN_COLOR){c.fillStyle=COIN_COLOR;c.fill();if(DRAW_SCORE)if(CENTER_SCORE){c.textAlign="center";c.fillText("$"+(ZERO-(y/r|0)),x-X,y-Y-r*2)}else c.fillText("$"+(ZERO-(y/r|0)),x-X-r,y-Y-r*2);c.fillStyle=BACKGROUND_COLOR;if(COIN_SYMBOL)c.fillText(COIN_SYMBOL,x-X-r/2,y-Y+r/2)}else{if(BACKGROUND_COLOR)c.fillStyle=BACKGROUND_COLOR;c.stroke()}})(0);
